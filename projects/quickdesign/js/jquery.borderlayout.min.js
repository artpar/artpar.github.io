...
$P.addClass("open");if(s.contentSelector){$C=$Cs[pane]=$P.children(s.contentSelector+":first");if(!$C.length){$Cs[pane]=false;return true;}$C.css(c.content.cssReq);if(s.applyDefaultStyles)$C.css(c.content.cssDef);$P.css({overflow:"hidden"});}});};var initHandles=function(){$.each(c.borderPanes.split(","),function(){var
...
$P=$Ps[pane],$R=$Rs[pane],$T=$Ts[pane],s=o[pane],doFX=(s.fxName!="none")&&!s.isClosed,edge=c[pane].edge,rClass=s.resizerClass,tClass=s.togglerClass,_side="-"+pane,_open="-open",_closed="-closed";if(!$P||(!s.resizable&&!s.closable))return;else if(!force&&s.isClosed)return;s.isClosed=true;syncPinBtns(pane,false);if(!s.isSliding)sizeMidPanes(c[pane].dir=="horz"?"all":"center");if($R){$R.css(edge,cDims[edge]).removeClass(rClass+_open+" "+rClass+_side+_open).addClass(rClass+_closed+" "+rClass+_side+_closed);if(s.resizable)$R.draggable("disable").css("cursor","default").attr("title","");if($T){$T.removeClass(tClass+_open+" "+tClass+_side+_open).addClass(tClass+_closed+" "+tClass+_side+_closed).attr("title",s.togglerTip_closed);}sizeHandles();}if(doFX){lockPaneForFX(pane,true);$P.hide(s.fxName,s.fxSettings,s.fxSpeed,function(){lockPaneForFX(pane,false);if(!s.isClosed)return;close_2();});}else{$P.hide();close_2();}function close_2(){bindStartSlidingEvent(pane,true);execCallback(pane,s.onclose);}};var open=function(pane,slide){var
$P=$Ps[pane],$R=$Rs[pane],$T=$Ts[pane],s=o[pane],fx=s.fx,doFX=(s.fxName!="none")&&s.isClosed,edge=c[pane].edge,rClass=s.resizerClass,tClass=s.togglerClass,_side="-"+pane,_open="-open",_closed="-closed";if(!$P||(!s.resizable&&!s.closable))return;else if(!s.isClosed&&!s.isSliding)return;if(s.isSliding&&!slide)bindStopSlidingEvents(pane,false);s.isClosed=false;s.isHidden=false;bindStartSlidingEvent(pane,false);if(doFX){lockPaneForFX(pane,true);$P.show(s.fxName,s.fxSettings,s.fxSpeed,function(){lockPaneForFX(pane,false);if(s.isClosed)return;open_2();});}else{$P.show();open_2();}function open_2(){if(!s.isSliding)sizeMidPanes(c[pane].dir=="vert"?"center":"all");if($R){$R.css(edge,cDims[edge]+getPaneSize(pane)).removeClass(rClass+_closed+" "+rClass+_side+_closed).addClass(rClass+_open+" "+rClass+_side+_open);if(s.resizable)$R.draggable("enable").css("cursor",s.resizerCursor).attr("title",s.resizerTip);else
...
$T.css({height:max(0,cssH($T,togLen)),width:max(1,cssW($T,spacing)),top:offset});}if(onInit&&s.initHidden){$R.hide();if($T)$T.hide();}});};var resizeAll=function(){cDims=getContainerDims();sizeMidPanes("all");sizeHandles("all");};function allowOverflow(elem){var $P;if(typeof elem=="string")$P=$Ps[elem]else
$P=$(elem).parents(".ui-layout-pane:first");if(!$P.length)return;var
pane=$P.attr("pane"),s=o[pane];if(s.isSliding||s.isResizing||s.isClosed){c[pane].cssSaved={};return;}var
newCSS={zIndex:(c.zIndex.pane_normal+1)},curCSS={},of=$P.css("overflow"),ofX=$P.css("overflowX"),ofY=$P.css("overflowY");if(of!="visible"){curCSS.overflow=of;newCSS.overflow="visible";}if(ofX&&ofX!="visible"&&ofX!="auto"){curCSS.overflowX=ofX;newCSS.overflowX="visible";}if(ofY&&ofY!="visible"&&ofY!="auto"){curCSS.overflowY=ofX;newCSS.overflowY="visible";}c[pane].cssSaved=curCSS;$P.css(newCSS);};function resetOverflow(elem){var $P;if(typeof elem=="string")$P=$Ps[elem]else
$P=$(elem).parents(".ui-layout-pane:first");if(!$P.length)return;var
...
$Container=$(this).css({overflow:"hidden"}),$Ps={},$Cs={},$Rs={},$Ts={},cDims={},c=config,o=options;create();return{containerDimensions:cDims,options:options,panes:$Ps,toggle:toggle,open:open,close:close,hide:hide,show:show,size:sizePane,resizeAll:resizeAll,addToggleBtn:addToggleBtn,addOpenBtn:addOpenBtn,addCloseBtn:addCloseBtn,addPinBtn:addPinBtn,allowOverflow:allowOverflow,resetOverflow:resetOverflow}}})(jQuery);